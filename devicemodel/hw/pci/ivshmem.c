	       ...
	       int baridx, uint64_t offset, int size, uint64_t value)
{
	...
	if (baridx == IVSHMEM_MMIO_BAR) {
		switch (offset) {
		/*
		 * For Revision 0, using IVSHMEM_IRQ_MASK_REG and
		 * IVSHMEM_IRQ_STA_REG to control legacy interrupt.
		 *
		 * For revision 1, both of IVSHMEM_IRQ_MASK_REG and
		 * IVSHMEM_IRQ_STA_REG are reserved, the interrupt
		 * uses MSI-X. Currently, we support revision 1 in here.
		 */
